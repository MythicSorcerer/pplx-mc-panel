...
export function ConsoleView() {
  ...
  function handleSubmit(e:FormEvent) { e.preventDefault(); const cmd=input.trim(); if(!cmd) return; send(cmd); setInput(""); }
  return (
      ...
      <div className="flex-1 overflow-hidden p-4 lg:p-8 grid xl:grid-cols-[1fr_280px] gap-4">
        <section className="terminal rounded-[28px] p-5 flex flex-col min-h-[400px]">
          ...
          <form onSubmit={handleSubmit} className="mt-4 flex gap-3 flex-shrink-0">
            <input className="flex-1 rounded-2xl bg-white/5 border border-white/10 px-4 py-3 text-white outline-none focus:border-mint/50 transition-colors font-mono text-sm"
              placeholder="say Hello world" value={input} onChange={e=>setInput(e.target.value)} aria-label="Console command input"/>
        ...
        <aside className="panel-line rounded-[28px] p-5 overflow-auto scrollbar-thin flex flex-col gap-4">
          <div>
            <p className="text-[10px] uppercase tracking-[0.3em] text-moss mb-3">Presets</p>
            <div className="grid gap-2">
              {PRESETS.map(cmd=>(
                <button key={cmd} onClick={()=>setInput(cmd)} className="rounded-2xl bg-white/5 hover:bg-white/10 px-4 py-3 text-left font-mono text-sm transition-colors">{cmd}</button>
              ))}
            </div>
          </div>
